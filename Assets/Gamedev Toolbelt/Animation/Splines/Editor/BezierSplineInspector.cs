...
public class BezierSplineInspector : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        else if (GUILayout.Button("Remove Curve"))
        {
            Undo.RecordObject(_spline, "Remove Curve");
            _spline.RemoveCurve(_selectedIndex);
            EditorUtility.SetDirty(_spline);
        }
    ...
    private void DrawSelectedPointInspector()
    {
        ...
        EditorGUI.BeginChangeCheck();
        BezierControlPointMode mode = _spline.GetControlPointMode(_selectedIndex);
        GUILayout.Label("Current mode: " + _spline.GetControlPointMode(_selectedIndex));
        GUILayout.BeginHorizontal();
        if (GUILayout.Button("Free"))
        {
            mode = BezierControlPointMode.FREE;
        }
        else if (GUILayout.Button("Aligned"))
        {
            mode = BezierControlPointMode.ALIGNED;
        }
        else if (GUILayout.Button("Mirrored"))
        {
